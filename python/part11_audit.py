...
class AuditTrailEngine:
    ...
    def verify_chain_integrity(self) -> dict:
        ...
        cols = [
            d[0]
            for d in self.con.execute("SELECT * FROM audit_trail LIMIT 0").description
        ]
...
class ESignatureEngine:
    ...
    def sign_record(
        ...
        cols = [
            d[0] for d in self.con.execute("SELECT * FROM users LIMIT 0").description
        ]
    ...
    def verify_signature(self, sig_id: str, current_record: dict) -> dict:
        ...
        cols = [
            d[0]
            for d in self.con.execute("SELECT * FROM esignatures LIMIT 0").description
        ]
    ...
    def authenticate(
        ...
        def _fail(uid, uname, role):
            ...
            if attempts >= MAX_LOGIN_ATTEMPTS:
                self.con.execute("UPDATE users SET locked=1 WHERE user_id=?", (uid,))
                self.audit.log(
                    uid,
                    uname,
                    role,
...
if __name__ == "__main__":
    ...
    dm_id = um.create_user(
        "cdm_raj", "Raj Kumar", Role.DATA_MANAGER, "raj@cro.com", "CdmRaj@Trial2024!"
    )
