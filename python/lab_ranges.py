...
def evaluate_lab_value(test_code: str, value: float, gender: str, age: float):
    ...
    if value < alert_low:
        return "CRITICALLY_LOW",  "<<", f"{test_code}={value} {unit} CRITICALLY LOW (ref: {low}–{high}, panic <{alert_low})"
    if value > alert_high:
        return "CRITICALLY_HIGH", ">>", f"{test_code}={value} {unit} CRITICALLY HIGH (ref: {low}–{high}, panic >{alert_high})"
    if value < low:
...
def evaluate_all_subjects():
    ...
    for _, row in df.iterrows():
        ...
        lab_tests = [
            ("HGB", row.get("Lab_Hb")),
            ("WBC", row.get("Lab_WBC") / 1000 if not pd.isna(row.get("Lab_WBC", float("nan"))) else None),
        ]
        ...
        for test_code, value in lab_tests:
            ...
            conn.execute("""
                ...
                VALUES (?,?,?,?,?,?,?,?,?,?,?)
            """, (usubjid, siteid, test_code, value,
                  ref[2] if ref else "",
                  ref[0] if ref else None,
                  ref[1] if ref else None,
                  status, flag, message, datetime.now().isoformat()))
...
def print_lab_report():
    ...
    if crit > 0:
        ...
        for _, row in crit_df.iterrows():
            print(f"  🚨 {row['Subject']} | {row['Test']} = {row['Value']} | {row['Message']}")
    ...
    if flagged.empty:
        ...
        for _, row in flagged.iterrows():
            ...
            print(f"  {icon} {row['Subject']:8} | {row['Test']:6} | {row['Value']:6} | {row['Message']}")
    print()
...
def print_reference_table():
    ...
    print("="*75)
    print(f"  {'Test':<8} {'Gender':<8} {'Age Range':<12} {'Low':>8} {'High':>8} {'Unit':<12} {'Panic Low':>10} {'Panic High':>10}")
    print("  " + "-"*71)
    for test, ranges in LAB_RANGES.items():
        for (gender, age_min, age_max), (lo, hi, unit, al, ah) in ranges.items():
            print(f"  {test:<8} {gender:<8} {age_min:>3}–{age_max:<7} {lo:>8} {hi:>8} {unit:<12} {al:>10} {ah:>10}")
    print()
