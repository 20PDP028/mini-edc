...
def get_sites():
    ...
    if not conn:
        return []
    with conn:
        rows = conn.execute("SELECT DISTINCT siteid FROM subjects ORDER BY siteid").fetchall()
...
def score_site(conn, siteid):
    ...
    score += pts
    if crit:
        factors.append({"factor": "Critical open queries", "count": crit, "points": pts, "level": "HIGH"})
    ...
    score += pts
    if sae:
        factors.append({"factor": "SAEs pending 24hr report", "count": sae, "points": pts, "level": "CRITICAL"})
    ...
    score += pts
    if major:
        factors.append({"factor": "Major open queries", "count": major, "points": pts, "level": "MEDIUM"})
    ...
    score += pts
    if stale:
        factors.append({"factor": "Stale queries (7+ days)", "count": stale, "points": pts, "level": "MEDIUM"})
    ...
    score += pts
    if pts:
        factors.append({"factor": f"Unanswered rate ({unans_rate:.0f}%)", "count": open_q, "points": pts, "level": "LOW"})
...
def run_risk_assessment():
    ...
    try:
        ...
        # Header
        headers = ["Site ID", "Risk Score", "Category", "Action Required",
                   "Subjects", "Critical Queries", "SAEs Pending", "Stale Queries", "Unans. Rate %"]
        ...
        for row_n, r in enumerate(results, 2):
            fill = fill_high if r["score"] >= 60 else (fill_medium if r["score"] >= 30 else fill_low)
            vals = [r["siteid"], r["score"], r["category"], r["action"],
                    r["subjects"], r["critical_queries"], r["sae_pending"],
                    r["stale_queries"], r["unanswered_rate"]]
