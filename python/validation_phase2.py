...
def validate(df):
    ...
    for idx, row in df.iterrows():
        ...
        if pd.isna(age):
            ...
            add_issue(row, "Age", "Critical", f"Age {age} out of range (1–120) for {subj}")
        ...
        if pd.isna(weight):
            ...
            add_issue(row, "Weight_kg", "Critical", f"Weight {weight} kg out of range for {subj}")
        ...
        if pd.isna(vdate) or str(vdate).strip() == "":
            ...
            try:
                ...
                add_issue(row, "Visit_Date", "Major", f"Invalid date format '{vdate}' for {subj}")
        ...
        if pd.isna(dose):
            ...
            add_issue(row, "Dose_mg", "Critical", f"Dose {dose} mg must be > 0 for {subj}")

        # ── Lab Hb ───────────────────────────────────────────
        hb = row.get("Lab_Hb")
        if not pd.isna(hb):
            if not (5 <= float(hb) <= 20):
                add_issue(row, "Lab_Hb", "Critical", f"Haemoglobin {hb} g/dL out of range (5–20) for {subj}")

        # ── Lab WBC ──────────────────────────────────────────
        wbc = row.get("Lab_WBC")
        if not pd.isna(wbc):
            if not (1000 <= float(wbc) <= 20000):
                add_issue(row, "Lab_WBC", "Major", f"WBC {wbc} out of range (1000–20000) for {subj}")
