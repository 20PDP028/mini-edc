...
def get_subjects(siteid):
    try:
        ...
        cur.execute(
            f"SELECT usubjid FROM subjects WHERE siteid = {ph} ORDER BY usubjid",
            (siteid,),
        )
...
def render_data_entry(user):
    ...
    if not subjects:
        ...
        subject_id = st.text_input(
            "Subject ID (manual entry)", placeholder="e.g. SUBJ-001"
        )
    else:
    ...
    with tab1:
        ...
        with col1:
            ...
            race = st.selectbox(
                "Race", ["", "Asian", "Black", "White", "Hispanic", "Other", "Unknown"]
            )
        with col2:
            weight_kg = st.number_input(
                "Weight (kg)", min_value=0.0, max_value=500.0, step=0.1, format="%.1f"
            )
            height_cm = st.number_input(
                "Height (cm)", min_value=0.0, max_value=300.0, step=0.1, format="%.1f"
            )
        ...
        if st.button("💾 Save Demographics", type="primary"):
            if not subject_id:
                ...
                log_audit(
                    "DATA_ENTRY", f"Demographics saved for {subject_id}", username
                )
    ...
    with tab2:
        ...
        with col1:
            bp_sys = st.number_input(
                "BP Systolic (mmHg)", min_value=0, max_value=300, step=1
            )
            bp_dia = st.number_input(
                "BP Diastolic (mmHg)", min_value=0, max_value=200, step=1
            )
        with col2:
        ...
        with col3:
            ...
            spo2 = st.number_input(
                "SpO₂ (%)", min_value=0.0, max_value=100.0, step=0.1, format="%.1f"
            )
        ...
        if st.button("💾 Save Vitals", type="primary"):
            if not subject_id:
                ...
                log_audit(
                    "DATA_ENTRY", f"Vitals saved for {subject_id} at {visit}", username
                )
    ...
    with tab3:
        ...
        with col1:
            result = st.number_input(
                f"Result ({ref['unit']})", min_value=0.0, step=0.01, format="%.2f"
            )
        with col2:
        ...
        if st.button("💾 Save Lab Result", type="primary"):
            if not subject_id:
                ...
                log_audit(
                    "DATA_ENTRY",
                    f"Lab {test_name}={result} saved for {subject_id}",
                    username,
                )
                if flag != "NORMAL":
    ...
    with tab4:
        ...
        with col2:
            ...
            compliance = st.selectbox(
                "Compliance", ["Full", "Partial", "Non-compliant", "Unknown"]
            )

        notes = st.text_area(
            "Notes",
            placeholder="Any relevant observations about medication administration...",
        )
        ...
        if st.button("💾 Save Medication", type="primary"):
            if not subject_id:
                ...
                log_audit(
                    "DATA_ENTRY",
                    f"Medication {drug_name} saved for {subject_id}",
                    username,
                )
                if compliance != "Full":
                    st.warning(
                        f"⚠️ Compliance recorded as '{compliance}' — consider raising a query."
                    )
                else:
