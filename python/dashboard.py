...
if not st.session_state.logged_in:
    st.markdown(
        "<div style='text-align:center;padding:40px 0 10px 0;'><div style='font-family:IBM Plex Mono,monospace;font-size:2rem;color:#4A9EDB;font-weight:600;'>🏥 Mini EDC</div><div style='color:#4A7AAF;font-size:0.85rem;letter-spacing:0.15em;text-transform:uppercase;margin-top:6px;'>Clinical Data Management System</div></div>",
        unsafe_allow_html=True,
    )
    ...
    with col_c:
        ...
        if st.button("🔐  Sign In", use_container_width=True, type="primary"):
            demo_users = {
                ...
                "SITE_001": {
                    "user_id": "SITE_001",
                    "full_name": "Site Staff A",
                    "role": "SITE",
                    "permissions": ["view_dashboard", "view_queries", "answer_query"],
                },
                "ADMIN": {
                    "user_id": "ADMIN",
        ...
        st.markdown(
            """<div style='margin-top:20px;padding:14px;background:#0D1F3C;border-radius:8px;border:1px solid #1E3A5F;font-size:0.78rem;color:#4A7AAF;'>
...
with st.sidebar:
    role = user["role"]
    st.markdown(
        f"""<div style='padding:12px 0 4px 0;'>
        ...
        <div style='font-size:0.7rem;color:#4A7AAF;margin-top:6px;'>{user["user_id"]}</div></div>""",
        unsafe_allow_html=True,
    )
...
if page == "📊 Dashboard":
    st.markdown(
        f"<h1 style='font-family:IBM Plex Mono,monospace;font-size:1.6rem;color:#E8EDF5;'>CDM Dashboard</h1><div style='color:#4A7AAF;font-size:0.8rem;margin-bottom:24px;'>Welcome back, {user['full_name']} · {datetime.now().strftime('%d %b %Y %H:%M')}</div>",
        unsafe_allow_html=True,
    )
    ...
    for col, val, label, color in zip(
        ...
        col.markdown(
            f"<div class='kpi-card'><div class='kpi-value' style='color:{color};'>{val}</div><div class='kpi-label'>{label}</div></div>",
            unsafe_allow_html=True,
        )
    ...
    with cl:
        st.markdown(
            "<div class='section-header'>Query Status</div>", unsafe_allow_html=True
        )
        if not df_queries.empty and "status" in df_queries.columns:
            st.bar_chart(
                df_queries["status"].value_counts(), color="#4A9EDB", height=200
            )
    with cr:
        st.markdown(
            "<div class='section-header'>Severity</div>", unsafe_allow_html=True
        )
        if not df_queries.empty and "severity" in df_queries.columns:
            st.bar_chart(
                df_queries["severity"].value_counts(), color="#C62828", height=200
            )

    if pending_sae > 0:
        st.markdown(
            "<div class='section-header'>⚠️ SAE Alerts</div>", unsafe_allow_html=True
        )
        for _, r in df_saes[df_saes["report_flag"] == "PENDING"].iterrows():
            st.markdown(
                f"<div class='alert-sae'>🚨 <b>SAE PENDING</b> | Subject: <b>{r.get('usubjid','')}</b> | Site: {r.get('siteid','')} | Event: <b>{r.get('aeterm','')}</b> | Severity: {r.get('aesev','')} | Date: {r.get('aestdtc','')}</div>",
                unsafe_allow_html=True,
            )

    st.markdown(
        "<div class='section-header'>Recent Open Queries</div>", unsafe_allow_html=True
    )
    open_df = (
        df_queries[df_queries["status"] == "Open"]
        if "status" in df_queries.columns
        else df_queries
    )
...
# ── Query Management ──────────────────────────────────────────
    ...
    st.markdown(
        "<h1 style='font-family:IBM Plex Mono,monospace;font-size:1.6rem;color:#E8EDF5;'>Query Management</h1>",
        unsafe_allow_html=True,
    )
    ...
    sf = c1.selectbox(
        "Status",
        (
            ["All"] + list(df_queries["status"].unique())
            if "status" in df_queries.columns
            else ["All"]
        ),
    )
    sv = c2.selectbox(
        "Severity",
        (
            ["All"] + list(df_queries["severity"].unique())
            if "severity" in df_queries.columns
            else ["All"]
        ),
    )
    ss = c3.selectbox(
        "Site",
        (
            ["All"] + list(df_queries["siteid"].unique())
            if "siteid" in df_queries.columns
            else ["All"]
        ),
    )
    ...
    if can("answer_query") or can("close_query"):
        st.markdown(
            "<div class='section-header'>Update Query Status</div>",
            unsafe_allow_html=True,
        )
        ca, cb, cc = st.columns(3)
        sel_q = ca.selectbox(
            "Query",
            df_queries["query_id"].tolist() if "query_id" in df_queries.columns else [],
        )
        statuses = (["Answered"] if can("answer_query") else []) + (
            ["Closed"] if can("close_query") else []
        )
        ...
        if new_status == "Closed":
            ...
            sig_meaning = st.text_input(
                "Signature Meaning",
                value="I certify this query has been reviewed and resolved",
            )
        ...
        if st.button("✅ Update Query", type="primary"):
            if _db_available():
                ...
                if new_status == "Closed" and AUTH_AVAILABLE:
                    ok, msg = esign(
                        user["user_id"], sig_pwd, "QUERY_CLOSE", sel_q, sig_meaning
                    )
...
# ── SAE Monitor ───────────────────────────────────────────────
    ...
    st.markdown(
        "<h1 style='font-family:IBM Plex Mono,monospace;font-size:1.6rem;color:#E8EDF5;'>SAE Monitor</h1>",
        unsafe_allow_html=True,
    )
    pending = (
        df_saes[df_saes["report_flag"] == "PENDING"]
        if "report_flag" in df_saes.columns
        else pd.DataFrame()
    )
    c1, c2 = st.columns(2)
...
# ── Audit Trail ───────────────────────────────────────────────
    ...
    st.markdown(
        "<h1 style='font-family:IBM Plex Mono,monospace;font-size:1.6rem;color:#E8EDF5;'>21 CFR Part 11 — Audit Trail</h1>",
        unsafe_allow_html=True,
    )
    ...
    if search:
        mask = (
            disp.astype(str)
            .apply(lambda c: c.str.contains(search, case=False))
            .any(axis=1)
        )
        disp = disp[mask]
...
# ── E-Signatures ──────────────────────────────────────────────
    ...
    st.markdown(
        "<h1 style='font-family:IBM Plex Mono,monospace;font-size:1.6rem;color:#E8EDF5;'>Electronic Signatures — 21 CFR Part 11</h1>",
        unsafe_allow_html=True,
    )
    st.markdown(
        "<div style='color:#4A7AAF;font-size:0.82rem;margin-bottom:20px;'>All signatures are cryptographically verified and immutable.</div>",
        unsafe_allow_html=True,
    )

    if can("close_query"):
        st.markdown(
            "<div class='section-header'>Create E-Signature</div>",
            unsafe_allow_html=True,
        )
        ca, cb = st.columns(2)
        with ca:
            sig_action = st.selectbox(
                "Action", ["QUERY_CLOSE", "DATA_LOCK", "REPORT_APPROVAL", "SAE_REVIEW"]
            )
            sig_record = st.text_input("Record ID", placeholder="e.g. QRY-0001")
        with cb:
            sig_meaning = st.text_input(
                "Meaning", value="I approve this record as accurate and complete"
            )
    ...
    if AUTH_AVAILABLE and _db_available():
        ...
        if sigs:
            for s in sigs:
                st.markdown(
                    f"<div class='sig-box'>✍️ <b>{s[2]}</b> <span style='color:#4A7AAF;'>({s[3]})</span> · <b>{s[4]}</b> on <code>{s[5]}</code> · <i>\"{s[6]}\"</i><br><span style='color:#4A7AAF;font-size:0.75rem;'>Signed: {s[7]} · Password verified: {'✅' if s[8] else '❌'}</span></div>",
                    unsafe_allow_html=True,
                )
        else:
...
# ── Generate PDF ──────────────────────────────────────────────
    ...
    st.markdown(
        "<h1 style='font-family:IBM Plex Mono,monospace;font-size:1.6rem;color:#E8EDF5;'>Generate PDF Report</h1>",
        unsafe_allow_html=True,
    )
    c1, c2 = st.columns(2)
    with c1:
    ...
    with c2:
        generated_by = st.text_input(
            "Generated By", value=f"{user['full_name']} ({user['role']})"
        )
    ...
    if st.button("📄 Generate PDF", type="primary"):
        try:
            ...
            generate_pdf(
                ...
                trial_name,
                df_queries.rename(
                    columns={"field_name": "field", "issue_description": "issue"}
                ).to_dict(orient="records"),
                df_saes.to_dict(orient="records"),
                df_audit.rename(columns={"field_name": "field"}).to_dict(
                    orient="records"
                ),
                generated_by,
                protocol,
...
# ── User Management ───────────────────────────────────────────
    ...
    if AUTH_AVAILABLE and _db_available():
        ...
        st.dataframe(
            pd.DataFrame(
                rows, columns=["User ID", "Full Name", "Role", "Active", "Last Login"]
            ),
            use_container_width=True,
            hide_index=True,
        )
    st.markdown(
        "<div class='section-header'>Role Permissions</div>", unsafe_allow_html=True
    )
    if AUTH_AVAILABLE:
