...
class SubjectIn(BaseModel):
    usubjid: str = Field(
        ...,
        example="STUDY001-001",
        description="Unique Subject Identifier (CDISC USUBJID)",
    )
...
class SDTMRequest(BaseModel):
    ...
    subjects: Optional[List[str]] = Field(
        None, example=["STUDY001-001", "STUDY001-002"]
    )
...
class QueryIn(BaseModel):
    ...
    message: str = Field(
        ...,
        example="Systolic value 185 seems implausibly high. Please verify source document.",
    )
        ...
        if domain == "DM":
                ...
                try:
                    if not (0 < int(age) <= 120):
                        add(
                            ...
                            uid,
                            "ERROR",
                            f"AGE {age} outside plausible range (1–120)",
                            "AGE",
                        )
                except ValueError:
            ...
            if (
                ...
                add(
                    ...
                    uid,
                    "ERROR",
                    "AESDTH=Y but AEOUT is not FATAL — inconsistency",
                    "AEOUT",
                )
                ...
                try:
                    if not (20 <= float(hr) <= 300):
                        add(
                            ...
                            uid,
                            "ERROR",
                            f"Heart rate {hr} outside plausible range (20–300)",
                            "VSORRES",
                        )
                except ValueError:
            ...
            try:
                ...
                if alt > 3 * uln_alt:
                    add(
                        ...
                        uid,
                        "CRITICAL",
                        f"ALT {alt} > 3× ULN ({uln_alt}) — hepatotoxicity signal",
                        "LBORRES",
                    )
            except ValueError:
                pass
            try:
                ...
                if ast > 3 * uln_ast:
                    add(
                        ...
                        uid,
                        "CRITICAL",
                        f"AST {ast} > 3× ULN ({uln_ast}) — hepatotoxicity signal",
                        "LBORRES",
                    )
            except ValueError:
                pass
            ...
            if exst and consent and exst < consent:
                add(
                    ...
                    uid,
                    "CRITICAL",
                    "Dose administered before consent — protocol violation",
                    "EXSTDTC",
                )
            dose = r.get("EXDOSE")
            if dose is not None:
...
def login(req: LoginRequest):
    ...
    if user["password"] != req.password:
        ...
        if user["fails"] >= 3:
            user["locked"] = True
        raise HTTPException(
            status_code=401, detail=f"Wrong password. Attempts: {user['fails']}/3"
        )
    user["fails"] = 0
...
def list_subjects(
    ...
    status: Optional[str] = Query(
        None, description="Filter by status (ENROLLED, COMPLETED, WITHDRAWN)"
    ),
...
@app.post(
    ...
    tags=["queries"],
    summary="Raise a data query against a subject record",
    status_code=201,
)
...
def respond_to_query(
    ...
    response: str = Body(
        ...,
        embed=True,
        example="Verified against source document. Value confirmed correct.",
    ),
    current_user: dict = Depends(get_current_user),
):
...
def system_stats(current_user: dict = Depends(get_current_user)):
    return {
        "subjects": {
            ...
            "completed": sum(
                1 for s in SUBJECTS.values() if s["status"] == "COMPLETED"
            ),
            "withdrawn": sum(
                1 for s in SUBJECTS.values() if s["status"] == "WITHDRAWN"
            ),
        },
        "queries": {
            "total": len(QUERIES),
