# Mini EDC — Phase E: REST API with PostgreSQL persistence
...
def init_db():
    ...
    """)
    cur.execute("""
        CREATE TABLE IF NOT EXISTS edc_audit (
    ...
    """)
    cur.execute("""
        CREATE TABLE IF NOT EXISTS edc_tokens (
    ...
    """)
    # Seed token table with users
    for uname in ['admin','investigator','datamanager','monitor']:
        cur.execute(f"""
            INSERT INTO edc_tokens (username, token, fails, locked)
            VALUES ({PH},{PH},{PH},{PH})
...
""",
    version="1.0.0",
    contact={"name": "Mini EDC Project", "email": "sponsor@cro.com"},
...
# In-memory user store (passwords only — tokens/locks now in DB)
USERS = {
    "admin":       {"password": "Admin@1234",   "role": "Admin"},
    "investigator":{"password": "Invest@1234",  "role": "Investigator"},
    "datamanager": {"password": "DataMgr@1234", "role": "Data Manager"},
    "monitor":     {"password": "Monitor@1234", "role": "Monitor"},
}
...
def get_current_user(token: str = Query(..., description="Auth token from /auth/login")):
    row = db_exec(f"SELECT username, locked FROM edc_tokens WHERE token={PH}", (token,), fetchone=True)
    if not row:
    ...
    return {"username": uname, "role": user_info.get("role", "Unknown")}

def log_audit(user: str, action: str, domain: str, usubjid: Optional[str], details: str):
    last = db_exec("SELECT chain_hash FROM edc_audit ORDER BY timestamp DESC LIMIT 1", fetchone=True)
    ...
    db_exec(
        f"INSERT INTO edc_audit (entry_id,timestamp,usr,action,domain,usubjid,details,chain_hash) VALUES ({PH},{PH},{PH},{PH},{PH},{PH},{PH},{PH})",
        (entry_id, ts, user, action, domain, usubjid, details, chain_hash),
        commit=True
    )
    return {"entry_id": entry_id, "timestamp": ts, "user": user, "action": action,
            "domain": domain, "usubjid": usubjid, "details": details, "chain_hash": chain_hash}
    ...
    for r in records:
        ...
        if domain == "DM":
            ...
            if hr:
                try:
                    if not (20 <= float(hr) <= 300):
                        add("VS002", uid, "ERROR", f"Heart rate {hr} outside plausible range", "VSORRES")
                except ValueError:
            ...
            try:
                ...
                if alt > 3 * uln_alt:
                    add("LB001", uid, "CRITICAL", f"ALT {alt} > 3× ULN — hepatotoxicity signal", "LBORRES")
            except ValueError:
                pass
            try:
                ...
                if ast > 3 * uln_ast:
                    add("LB002", uid, "CRITICAL", f"AST {ast} > 3× ULN — hepatotoxicity signal", "LBORRES")
            except ValueError:
                pass
        elif domain == "EX":
            exst = r.get("EXSTDTC","")
    ...
    for i, uid in enumerate(subs):
        if d == "DM":
            rows.append({"STUDYID":studyid,"DOMAIN":"DM","USUBJID":uid,"SUBJID":uid.split("-")[-1],
                         "SITEID":f"SITE-0{i+1}","AGE":40+i*5,"AGEU":"YEARS","SEX":["M","F","M"][i%3],
                         "RACE":"WHITE","ETHNIC":"NOT HISPANIC OR LATINO","COUNTRY":"USA",
                         "DMDTC":f"2024-01-{15+i:02d}","RFSTDTC":f"2024-01-{16+i:02d}","RFENDTC":"2024-06-30"})
        elif d == "AE":
            rows.append({"STUDYID":studyid,"DOMAIN":"AE","USUBJID":uid,"AESEQ":1,"AETERM":"HEADACHE",
                         "AEDECOD":"Headache","AEBODSYS":"NERVOUS SYSTEM DISORDERS",
                         "AESTDTC":f"2024-02-{10+i:02d}","AEENDTC":f"2024-02-{12+i:02d}",
                         "AESEV":"MILD","AESER":"N","AEREL":"POSSIBLY RELATED","AEOUT":"RECOVERED/RESOLVED","AESDTH":"N"})
        elif d == "VS":
            rows.append({"STUDYID":studyid,"DOMAIN":"VS","USUBJID":uid,"VSSEQ":1,"VSTESTCD":"SYSBP",
                         "VSTEST":"Systolic Blood Pressure","VSORRES":str(120+i*2),"VSORRESU":"mmHg",
                         "VSSTRESN":120+i*2,"VSSTRESU":"mmHg","VSBLFL":"Y","VISITNUM":1,
                         "VISIT":"BASELINE","VSDTC":f"2024-01-{16+i:02d}"})
        elif d == "LB":
            rows.append({"STUDYID":studyid,"DOMAIN":"LB","USUBJID":uid,"LBSEQ":1,"LBTESTCD":"ALT",
                         "LBTEST":"Alanine Aminotransferase","LBORRES":str(25+i*3),"LBORRESU":"U/L",
                         "LBSTRESN":25+i*3,"LBSTRESU":"U/L","LBNRLO":"7","LBNRHI":"40","LBNRIND":"NORMAL",
                         "LBBLFL":"Y","VISITNUM":1,"VISIT":"BASELINE","LBDTC":f"2024-01-{16+i:02d}"})
        elif d == "EX":
            rows.append({"STUDYID":studyid,"DOMAIN":"EX","USUBJID":uid,"EXSEQ":1,"EXTRT":"STUDY DRUG",
                         "EXDOSE":100,"EXDOSU":"mg","EXDOSFRQ":"QD","EXROUTE":"ORAL",
                         "EXSTDTC":f"2024-01-{16+i:02d}","EXENDTC":"2024-06-30"})
    return rows

# ── Routes: Auth ──────────────────────────────────────────────────────────────

@app.post("/auth/login", response_model=LoginResponse, tags=["auth"], summary="Login and receive an auth token")
def login(req: LoginRequest):
    user = USERS.get(req.username)
    if not user:
        raise HTTPException(status_code=401, detail="User not found")
    row = db_exec(f"SELECT fails, locked FROM edc_tokens WHERE username={PH}", (req.username,), fetchone=True)
    ...
    if user["password"] != req.password:
        ...
        locked = 1 if fails >= 3 else 0
        db_exec(f"UPDATE edc_tokens SET fails={PH}, locked={PH} WHERE username={PH}", (fails, locked, req.username), commit=True)
        raise HTTPException(status_code=401, detail=f"Wrong password. Attempts: {fails}/3")
    token = make_token(req.username)
    db_exec(f"UPDATE edc_tokens SET token={PH}, fails=0, locked=0 WHERE username={PH}", (token, req.username), commit=True)
    log_audit(req.username, "LOGIN", "SYSTEM", None, "Successful login")
    return {"token": token, "username": req.username, "role": user["role"], "message": f"Welcome, {req.username}!"}

@app.post("/auth/logout", tags=["auth"], summary="Logout and invalidate token")
def logout(current_user: dict = Depends(get_current_user)):
    db_exec(f"UPDATE edc_tokens SET token=NULL WHERE username={PH}", (current_user["username"],), commit=True)
...
@app.post("/subjects", response_model=SubjectOut, tags=["subjects"], summary="Enroll a new subject", status_code=201)
def create_subject(subject: SubjectIn, current_user: dict = Depends(get_current_user)):
    existing = db_exec(f"SELECT usubjid FROM edc_subjects WHERE usubjid={PH}", (subject.usubjid,), fetchone=True)
    if existing:
        raise HTTPException(status_code=409, detail=f"Subject {subject.usubjid} already enrolled")
    ts = datetime.utcnow().isoformat() + "Z"
    consent_str = subject.consent_date.isoformat()
    db_exec(
        f"INSERT INTO edc_subjects (usubjid,age,sex,race,country,consent_date,site_id,status,enrolled_at) VALUES ({PH},{PH},{PH},{PH},{PH},{PH},{PH},{PH},{PH})",
        (subject.usubjid, subject.age, subject.sex, subject.race, subject.country, consent_str, subject.site_id, "ENROLLED", ts),
        commit=True
    )
    log_audit(current_user["username"], "SUBJECT_ENROLL", "DM", subject.usubjid, f"Subject enrolled at site {subject.site_id}")
    return {**subject.dict(), "consent_date": consent_str, "status": "ENROLLED", "enrolled_at": ts}

@app.get("/subjects", response_model=List[SubjectOut], tags=["subjects"], summary="List all enrolled subjects")
def list_subjects(
...
def get_subject(usubjid: str, current_user: dict = Depends(get_current_user)):
    row = db_exec(f"SELECT * FROM edc_subjects WHERE usubjid={PH}", (usubjid,), fetchone=True)
    if not row:
        raise HTTPException(status_code=404, detail=f"Subject {usubjid} not found")
    return row

@app.patch("/subjects/{usubjid}/status", tags=["subjects"], summary="Update subject status")
def update_subject_status(
    usubjid: str,
    new_status: Literal["ENROLLED","COMPLETED","WITHDRAWN"] = Body(..., embed=True),
    current_user: dict = Depends(get_current_user)
):
    row = db_exec(f"SELECT status FROM edc_subjects WHERE usubjid={PH}", (usubjid,), fetchone=True)
    if not row:
        raise HTTPException(status_code=404, detail=f"Subject {usubjid} not found")
    old = row["status"]
    db_exec(f"UPDATE edc_subjects SET status={PH} WHERE usubjid={PH}", (new_status, usubjid), commit=True)
    log_audit(current_user["username"], "STATUS_CHANGE", "DS", usubjid, f"Status changed from {old} to {new_status}")
    return {"usubjid": usubjid, "old_status": old, "new_status": new_status}

# ── Routes: Queries ───────────────────────────────────────────────────────────

@app.post("/queries", response_model=QueryOut, tags=["queries"], summary="Raise a data query", status_code=201)
def raise_query(q: QueryIn, current_user: dict = Depends(get_current_user)):
    count = db_exec("SELECT COUNT(*) as c FROM edc_queries", fetchone=True)
    qid = f"QRY-{(count['c'] if count else 0)+1:04d}"
    ts = datetime.utcnow().isoformat() + "Z"
    db_exec(
        f"INSERT INTO edc_queries (query_id,usubjid,domain,field,visit,message,status,raised_by,raised_at,response,closed_at) VALUES ({PH},{PH},{PH},{PH},{PH},{PH},{PH},{PH},{PH},{PH},{PH})",
        (qid, q.usubjid, q.domain.value, q.field, q.visit, q.message, "OPEN", current_user["username"], ts, None, None),
        commit=True
    )
    log_audit(current_user["username"], "QUERY_RAISED", q.domain.value, q.usubjid, f"Query {qid} raised on {q.field} at {q.visit}")
    return {"query_id": qid, **q.dict(), "domain": q.domain.value, "status": "OPEN",
            "raised_by": current_user["username"], "raised_at": ts, "response": None, "closed_at": None}

@app.get("/queries", response_model=List[QueryOut], tags=["queries"], summary="List all queries")
def list_queries(
...
def respond_to_query(
    ...
    q = db_exec(f"SELECT * FROM edc_queries WHERE query_id={PH}", (query_id,), fetchone=True)
    if not q:
...
@app.post("/validation/run", response_model=ValidationResponse, tags=["validation"], summary="Run CDISC edit checks")
...
@app.get("/validation/rules", tags=["validation"], summary="List all validation rules")
...
@app.post("/sdtm/generate", tags=["sdtm"], summary="Generate an SDTM v1.8 dataset for a domain")
def generate_sdtm(req: SDTMRequest, current_user: dict = Depends(get_current_user)):
    ...
    return {"domain": req.domain.value, "row_count": len(data), "sdtm_version": "1.8", "rows": data}

@app.get("/sdtm/domains", tags=["sdtm"], summary="List supported SDTM domains")
...
@app.get("/audit/trail", response_model=List[AuditEntry], tags=["audit"], summary="Retrieve the 21 CFR Part 11 audit trail")
def get_audit_trail(
...
def verify_audit_integrity(current_user: dict = Depends(get_current_user)):
    ...
    return {"status": "INTACT" if broken == 0 else "TAMPERED",
            "entries_checked": len(rows), "broken_links": broken}
...
def health_check():
    return {"status": "healthy", "timestamp": datetime.utcnow().isoformat()+"Z",
            "version": "1.0.0", "phase": "E", "db": "postgres" if USE_POSTGRES else "none"}
...
def system_stats(current_user: dict = Depends(get_current_user)):
    subj = db_exec("SELECT status, COUNT(*) as c FROM edc_subjects GROUP BY status", fetchall=True) or []
    subj_map = {r["status"]: r["c"] for r in subj}
    qry = db_exec("SELECT status, COUNT(*) as c FROM edc_queries GROUP BY status", fetchall=True) or []
    qry_map = {r["status"]: r["c"] for r in qry}
    audit_count = db_exec("SELECT COUNT(*) as c FROM edc_audit", fetchone=True)
    return {
        "subjects": {"total": sum(subj_map.values()), "enrolled": subj_map.get("ENROLLED",0),
                     "completed": subj_map.get("COMPLETED",0), "withdrawn": subj_map.get("WITHDRAWN",0)},
        "queries":  {"total": sum(qry_map.values()), "open": qry_map.get("OPEN",0),
                     "answered": qry_map.get("ANSWERED",0), "closed": qry_map.get("CLOSED",0)},
        "audit_entries": audit_count["c"] if audit_count else 0,
        "users_online": db_exec("SELECT COUNT(*) as c FROM edc_tokens WHERE token IS NOT NULL", fetchone=True)["c"]
    }

@app.get("/", tags=["system"], summary="API root")
def root():
    return {"name": "Mini EDC REST API", "version": "1.0.0", "phase": "E",
            "docs": "/docs", "redoc": "/redoc", "health": "/health", "openapi_spec": "/openapi.json"}
