...
def get_current_user(
    token: str = Query(..., description="Auth token from /auth/login")
):
    row = db_exec(
        f"SELECT username, locked FROM edc_tokens WHERE token={PH}",
        (token,),
        fetchone=True,
    )
    if not row:
...
def log_audit(
    user: str, action: str, domain: str, usubjid: Optional[str], details: str
):
    last = db_exec(
        "SELECT chain_hash FROM edc_audit ORDER BY timestamp DESC LIMIT 1",
        fetchone=True,
    )
        ...
        if domain == "DM":
            if r.get("SEX", "").upper() not in SEX_CT:
                add(
                    ...
                    uid,
                    "ERROR",
                    f"SEX '{r.get('SEX')}' not in CDISC CT",
                    "SEX",
                )
            if r.get("RACE", "").upper() not in RACE_CT:
                add(
                    ...
                    uid,
                    "ERROR",
                    f"RACE '{r.get('RACE')}' not in CDISC CT",
                    "RACE",
                )
            if r.get("ETHNIC", "").upper() not in ETHNIC_CT:
                add(
                    ...
                    uid,
                    "WARNING",
                    f"ETHNIC '{r.get('ETHNIC')}' not in CDISC CT",
                    "ETHNIC",
                )
            age = r.get("AGE")
            if age is not None:
                try:
                    if not (0 < int(age) <= 120):
                        add(
                            ...
                            uid,
                            "ERROR",
                            f"AGE {age} outside plausible range",
                            "AGE",
                        )
                except ValueError:
            ...
            if rfst and consent and rfst < consent:
                add(
                    ...
                    uid,
                    "CRITICAL",
                    "First dose before consent date — protocol violation",
                    "RFSTDTC",
                )
        elif domain == "AE":
            ...
            if str(r.get("AESER", "")).upper() == "Y" and sev != "SEVERE":
                add(
                    ...
                    uid,
                    "WARNING",
                    "SAE flagged but severity is not SEVERE",
                    "AESER",
                )
            if (
                str(r.get("AESDTH", "")).upper() == "Y"
                and r.get("AEOUT", "").upper() != "FATAL"
            ):
                ...
                try:
                    if float(sys_bp) <= float(dia_bp):
                        add(
                            ...
                            uid,
                            "ERROR",
                            f"Systolic {sys_bp} ≤ Diastolic {dia_bp}",
                            "VSORRES",
                        )
                except ValueError:
                    pass
                ...
                try:
                    if not (20 <= float(hr) <= 300):
                        add(
                            ...
                            uid,
                            "ERROR",
                            f"Heart rate {hr} outside plausible range",
                            "VSORRES",
                        )
                except ValueError:
            ...
            try:
                ...
                if alt > 3 * uln_alt:
                    add(
                        ...
                        uid,
                        "CRITICAL",
                        f"ALT {alt} > 3× ULN — hepatotoxicity signal",
                        "LBORRES",
                    )
            except ValueError:
                pass
            try:
                ...
                if ast > 3 * uln_ast:
                    add(
                        ...
                        uid,
                        "CRITICAL",
                        f"AST {ast} > 3× ULN — hepatotoxicity signal",
                        "LBORRES",
                    )
            except ValueError:
                pass
            ...
            if exst and consent and exst < consent:
                add(
                    ...
                    uid,
                    "CRITICAL",
                    "Dose administered before consent",
                    "EXSTDTC",
                )
            dose = r.get("EXDOSE")
            if dose is not None:
            ...
            rows.append(
                {
                    ...
                    "COUNTRY": "USA",
                    "DMDTC": f"2024-01-{15+i:02d}",
                    "RFSTDTC": f"2024-01-{16+i:02d}",
                    "RFENDTC": "2024-06-30",
                }
            )
        elif d == "AE":
            rows.append(
                {
                    ...
                    "AESTDTC": f"2024-02-{10+i:02d}",
                    "AEENDTC": f"2024-02-{12+i:02d}",
                    "AESEV": "MILD",
                    "AESER": "N",
...
def login(req: LoginRequest):
    ...
    row = db_exec(
        f"SELECT fails, locked FROM edc_tokens WHERE username={PH}",
        (req.username,),
        fetchone=True,
    )
    if row and row["locked"]:
        raise HTTPException(
            status_code=403, detail="Account locked after 3 failed attempts"
        )
    if user["password"] != req.password:
        fails = (row["fails"] if row else 0) + 1
        locked = 1 if fails >= 3 else 0
        db_exec(
            f"UPDATE edc_tokens SET fails={PH}, locked={PH} WHERE username={PH}",
            (fails, locked, req.username),
            commit=True,
        )
        raise HTTPException(
            status_code=401, detail=f"Wrong password. Attempts: {fails}/3"
        )
    token = make_token(req.username)
    db_exec(
        f"UPDATE edc_tokens SET token={PH}, fails=0, locked=0 WHERE username={PH}",
        (token, req.username),
        commit=True,
    )
    ...
    return {
        ...
        "role": user["role"],
        "message": f"Welcome, {req.username}!",
    }
...
def logout(current_user: dict = Depends(get_current_user)):
    db_exec(
        f"UPDATE edc_tokens SET token=NULL WHERE username={PH}",
...
@app.post(
    ...
    tags=["subjects"],
    summary="Enroll a new subject",
    status_code=201,
)
def create_subject(subject: SubjectIn, current_user: dict = Depends(get_current_user)):
    existing = db_exec(
        f"SELECT usubjid FROM edc_subjects WHERE usubjid={PH}",
        (subject.usubjid,),
        fetchone=True,
    )
    if existing:
        raise HTTPException(
            status_code=409, detail=f"Subject {subject.usubjid} already enrolled"
        )
    ts = datetime.utcnow().isoformat() + "Z"
...
def get_subject(usubjid: str, current_user: dict = Depends(get_current_user)):
    row = db_exec(
        f"SELECT * FROM edc_subjects WHERE usubjid={PH}", (usubjid,), fetchone=True
    )
    if not row:
...
@app.patch(
    "/subjects/{usubjid}/status", tags=["subjects"], summary="Update subject status"
)
def update_subject_status(
    ...
    row = db_exec(
        f"SELECT status FROM edc_subjects WHERE usubjid={PH}", (usubjid,), fetchone=True
    )
    if not row:
        raise HTTPException(status_code=404, detail=f"Subject {usubjid} not found")
    old = row["status"]
    db_exec(
        f"UPDATE edc_subjects SET status={PH} WHERE usubjid={PH}",
        (new_status, usubjid),
        commit=True,
    )
    log_audit(
...
def respond_to_query(
    ...
    q = db_exec(
        f"SELECT * FROM edc_queries WHERE query_id={PH}", (query_id,), fetchone=True
    )
    if not q:
    ...
    db_exec(
        f"UPDATE edc_queries SET status='ANSWERED', response={PH} WHERE query_id={PH}",
        (response, query_id),
        commit=True,
    )
    log_audit(
...
def close_query(query_id: str, current_user: dict = Depends(get_current_user)):
    ...
    ts = datetime.utcnow().isoformat() + "Z"
    db_exec(
        f"UPDATE edc_queries SET status='CLOSED', closed_at={PH} WHERE query_id={PH}",
        (ts, query_id),
        commit=True,
    )
    log_audit(
...
@app.post(
    ...
    tags=["validation"],
    summary="Run CDISC edit checks",
)
def run_validation(
    req: ValidationRequest, current_user: dict = Depends(get_current_user)
):
    ...
    return {
        ...
        "records_checked": len(req.records),
    ...
    all_rules = [
        {
            "rule_id": "DM001",
            "domain": "DM",
            "check": "SEX controlled terminology",
            "severity": "ERROR",
        },
        {
            "rule_id": "DM002",
            "domain": "DM",
            "check": "RACE controlled terminology",
            "severity": "ERROR",
        },
        {
            "rule_id": "DM003",
            "domain": "DM",
            "check": "ETHNIC controlled terminology",
            "severity": "WARNING",
        },
        {
            "rule_id": "DM004",
            "domain": "DM",
            "check": "AGE plausibility (1–120 years)",
            "severity": "ERROR",
        },
        {
            "rule_id": "DM005",
            "domain": "DM",
            "check": "Consent before first dose",
            "severity": "CRITICAL",
        },
        {
            "rule_id": "AE001",
            "domain": "AE",
            "check": "AE end ≥ start date",
            "severity": "ERROR",
        },
        {
            "rule_id": "AE002",
            "domain": "AE",
            "check": "AESEV controlled terminology",
            "severity": "ERROR",
        },
        {
            "rule_id": "AE003",
            "domain": "AE",
            "check": "AEREL controlled terminology",
            "severity": "WARNING",
        },
        {
            "rule_id": "AE004",
            "domain": "AE",
            "check": "SAE severity cross-check",
            "severity": "WARNING",
        },
        {
            "rule_id": "AE005",
            "domain": "AE",
            "check": "AESDTH / AEOUT consistency",
            "severity": "ERROR",
        },
        {
            "rule_id": "VS001",
        ...
        },
        {
            "rule_id": "VS002",
            "domain": "VS",
            "check": "Heart rate plausibility (20–300)",
            "severity": "ERROR",
        },
        {
            "rule_id": "LB001",
            "domain": "LB",
            "check": "ALT > 3× ULN hepatotoxicity",
            "severity": "CRITICAL",
        },
        {
            "rule_id": "LB002",
            "domain": "LB",
            "check": "AST > 3× ULN hepatotoxicity",
            "severity": "CRITICAL",
        },
        {
            "rule_id": "EX001",
            "domain": "EX",
            "check": "Dose after consent date",
            "severity": "CRITICAL",
        },
        {
            "rule_id": "EX002",
            "domain": "EX",
            "check": "Non-negative dose value",
            "severity": "ERROR",
        },
    ]
    if domain:
...
@app.get(
    ...
    tags=["audit"],
    summary="Retrieve the 21 CFR Part 11 audit trail",
)
def get_audit_trail(
...
def verify_audit_integrity(current_user: dict = Depends(get_current_user)):
    rows = db_exec(
        "SELECT usr, action, domain, usubjid, details, chain_hash FROM edc_audit ORDER BY timestamp ASC",
        fetchall=True,
    )
    if not rows:
    ...
    return {
        "status": "INTACT" if broken == 0 else "TAMPERED",
...
def health_check():
    return {
        ...
        "version": "1.0.0",
        "phase": "E",
        "db": "postgres" if USE_POSTGRES else "none",
    }
...
def system_stats(current_user: dict = Depends(get_current_user)):
    subj = (
        db_exec(
            "SELECT status, COUNT(*) as c FROM edc_subjects GROUP BY status",
            fetchall=True,
        )
        or []
    )
    ...
    qry = (
        db_exec(
            "SELECT status, COUNT(*) as c FROM edc_queries GROUP BY status",
            fetchall=True,
        )
        or []
    )
    ...
    return {
        ...
        "users_online": db_exec(
            "SELECT COUNT(*) as c FROM edc_tokens WHERE token IS NOT NULL",
            fetchone=True,
        )["c"],
    }
