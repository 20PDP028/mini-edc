...
def answer_query(query_id):
    ...
    with get_db() as conn:
        q = conn.execute("SELECT status FROM queries WHERE query_id=?", (query_id,)).fetchone()
        if not q:
            return jsonify({"error": f"Query {query_id} not found"}), 404
        if q["status"] != "Open":
            return jsonify({"error": f"Query is '{q['status']}' — can only answer Open queries"}), 409
...
def close_query(query_id):
    ...
    with get_db() as conn:
        ...
        if q["status"] != "Answered":
            return jsonify({"error": f"Query is '{q['status']}' — can only close Answered queries"}), 409
        now = datetime.now().isoformat(sep=" ", timespec="seconds")
        conn.execute(
...
def summary():
    with get_db() as conn:
        q_open   = conn.execute("SELECT COUNT(*) FROM queries WHERE status='Open'").fetchone()[0]
        q_ans    = conn.execute("SELECT COUNT(*) FROM queries WHERE status='Answered'").fetchone()[0]
        q_closed = conn.execute("SELECT COUNT(*) FROM queries WHERE status='Closed'").fetchone()[0]
        q_crit   = conn.execute("SELECT COUNT(*) FROM queries WHERE severity='Critical' AND status='Open'").fetchone()[0]
        sae_pend = conn.execute("SELECT COUNT(*) FROM adverse_events WHERE aeser='Y' AND report_flag='PENDING'").fetchone()[0]
        subjects = conn.execute("SELECT COUNT(*) FROM subjects").fetchone()[0]
        sites    = conn.execute("SELECT COUNT(DISTINCT siteid) FROM subjects").fetchone()[0]
...
def audit_trail():
    if g.role not in ("Data Manager", "Administrator"):
        return jsonify({"error": "Only Data Managers and Admins can view audit trail"}), 403
...
if __name__ == "__main__":
    ...
    print(f"    GET  /api/subjects          ?siteid=SITE01")
    print(f"    GET  /api/subjects/<id>")
    print(f"    GET  /api/queries           ?status=Open&severity=Critical")
    print(f"    POST /api/queries/<id>/answer")
    print(f"    POST /api/queries/<id>/close")
    print(f"    GET  /api/saes")
    print(f"    POST /api/validate")
    print(f"    GET  /api/audit             ?limit=50")
    print(f"\n  API Keys (add header: X-API-Key: <key>):")
