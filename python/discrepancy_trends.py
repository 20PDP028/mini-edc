...
def plot_severity_by_site(df_q, ax):
    ...
    pivot.plot(kind="bar", ax=ax, stacked=True,
               ...
               edgecolor="white", linewidth=0.5)
    ax.set_title("Severity Breakdown by Site", fontsize=11, fontweight="bold", color=NAVY, pad=10)
    ax.set_xlabel("Site", fontsize=9, color="#555")
...
def plot_status_donut(df_q, ax):
    ...
    for at in autotexts:
        ...
        at.set_fontsize(9)
    ax.set_title("Query Status Distribution", fontsize=11, fontweight="bold", color=NAVY, pad=10)
...
def plot_top_error_fields(df_q, ax):
    ...
    for bar, val in zip(bars, counts.values[::-1]):
        ax.text(bar.get_width() + 0.05, bar.get_y() + bar.get_height()/2,
                str(val), va="center", fontsize=9, fontweight="bold", color=NAVY)
...
def plot_sae_by_site(df_s, ax):
    ...
    bars = ax.bar(counts.index, counts.values, color=RED,
                  edgecolor="white", linewidth=0.5)
    ...
    for bar, val in zip(bars, counts.values):
        ax.text(bar.get_x() + bar.get_width()/2, bar.get_height() + 0.02,
                str(val), ha="center", va="bottom", fontsize=9, fontweight="bold", color=RED)
...
def plot_critical_vs_major(df_q, ax):
    ...
    if df_q.empty or "siteid" not in df_q.columns:
        ax.text(0.5, 0.5, "No data", ha="center", va="center")
        return
    pivot = df_q[df_q["severity"].isin(["Critical","Major"])]\
        .groupby(["siteid","severity"]).size().unstack(fill_value=0)
    x = range(len(pivot))
    w = 0.35
    if "Critical" in pivot.columns:
        ax.bar([i - w/2 for i in x], pivot["Critical"], w, label="Critical", color=RED, edgecolor="white")
    if "Major" in pivot.columns:
        ax.bar([i + w/2 for i in x], pivot["Major"],    w, label="Major",    color=AMBER, edgecolor="white")
    ax.set_xticks(list(x))
    ax.set_xticklabels(pivot.index, fontsize=9)
    ax.set_title("Critical vs Major by Site", fontsize=11, fontweight="bold", color=NAVY, pad=10)
    ax.set_ylabel("Count", fontsize=9, color="#555")
    ax.set_facecolor(LGRAY)
    ax.tick_params(colors="#555")
...
def generate_trend_report():
    ...
    # Title banner
    fig.text(0.5, 0.97, "Clinical Trial — Discrepancy Trending Report",
             ha="center", va="top", fontsize=16, fontweight="bold", color=NAVY)
    fig.text(0.5, 0.945, f"Generated: {datetime.now().strftime('%d %b %Y %H:%M')}  |  Total Queries: {len(df_q)}  |  Total SAEs: {len(df_s)}",
    ...
    crit_q  = len(df_q[df_q["severity"]=="Critical"]) if "severity" in df_q.columns else 0
    pend_s  = len(df_s[df_s["report_flag"]=="PENDING"]) if "report_flag" in df_s.columns else 0
    fig.text(0.02, 0.08,
...
def print_site_summary():
    ...
    if not df_q.empty and "siteid" in df_q.columns:
        for site in df_q["siteid"].unique():
            ...
            crit = len(site_df[site_df["severity"]=="Critical"]) if "severity" in site_df.columns else 0
            maj  = len(site_df[site_df["severity"]=="Major"])    if "severity" in site_df.columns else 0
            opn  = len(site_df[site_df["status"]=="Open"])       if "status"   in site_df.columns else 0
            print(f"  {site:10} | Total: {len(site_df):3} | Critical: {crit:2} | Major: {maj:2} | Open: {opn:2}")
    print()
