...
class SDTMGenerator:
    ...
    def generate_dm(self, raw_dm: list[dict]) -> list[dict]:
        ...
        for rec in raw_dm:
            ...
            armcd = (
                "TRT-A"
                if "TREATMENT A" in arm.upper()
                else "PBO" if "PLACEBO" in arm.upper() else "UNK"
            )
            rows.append(
                {
...
class DefineXMLGenerator:
    ...
    def generate(self, output_path: str) -> str:
        ...
        for ds in self.datasets:
            ...
            for idx, (var_name, var_type, var_label, required) in enumerate(
                ds["variables"], 1
            ):
                lines.append(
            ...
            for rec in self.datasets.get(domain, []):
                ...
                if sid and sid not in dm_subjects:
                    self._issue(
                        domain,
                        "CONF-REF01",
                        f"USUBJID '{sid}' in {domain} not found in DM",
                        "ERROR",
                    )
            ...
            for rec in self.datasets.get(domain, []):
                ...
                if key in seen:
                    self._issue(
                        domain,
                        "CONF-SEQ01",
                        f"Duplicate {seq_field}={seq} for subject {sid}",
                        "ERROR",
                    )
                if seq:
                    seen[key] = True
            ...
            for rec in rows:
                if rec.get("STUDYID", "") == "":
                    self._issue(
                        domain,
                        "CONF-STDY01",
                        f"STUDYID missing in {domain} record",
                        "ERROR",
                    )
...
if __name__ == "__main__":
    ...
    for ds in gen.generated_datasets:
        print(
            f"   {ds['domain']:4s} — {ds['label']:35s} ({ds['n_records']} records) → {os.path.basename(ds['filepath'])}"
        )
    ...
    report = {
        ...
        "datasets": [
            {k: v for k, v in ds.items() if k != "variables"}
            for ds in gen.generated_datasets
        ],
